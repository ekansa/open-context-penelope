...
class dbXML_xmlMetadata  {
    ...
    public $doSocial = true; //make a social usage node, set to false if project
	 ...
	 public $persistenIDs; //array of persistent IDs (for individual items that usually don't get them)
    
	 const doiHrefRoot = "http://dx.doi.org/";
	 const arkHrefRoot = "http://n2t.net/ark:/";
    ...
    public function addMetadata(){
		  ...
		  if(is_array($metadata->persistentIDs)){
				foreach($metadata->persistentIDs as $stableID){
					 $stableLinkHref = false;
					 $elementB = $doc->createElement("dc:identifier");
					 $elementB->setAttribute("type", $stableID["stableType"]);
					 if(stristr($stableID["stableType"],"doi")){
						  $stableLinkHref = self::doiHrefRoot.$stableID["stableID"];
					 }
					 elseif(stristr($stableID["stableType"],"ark")){
						  $stableLinkHref = self::arkHrefRoot.$stableID["stableID"];
					 }
					 
					 if($stableLinkHref != false){
						  $elementB->setAttribute("href", $stableLinkHref);
					 }
					 $elementBtext = $doc->createTextNode($stableID["stableID"]);
					 $elementB->appendChild($elementBtext);
					 $element->appendChild($elementB);
