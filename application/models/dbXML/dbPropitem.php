...
class dbXML_dbPropitem  {
    ...
    */
    ...
    private $pen_obsItemTypes = array("space" => "location%",
				  "media" => "%media%",
				  "project" => "%project%",
				  "person" => "%person%",
				  "document" => "%diary%");
	...
	if($this->dbPenelope){
		...
		LEFT JOIN val_tab ON  val_tab.value_uuid = properties.value_uuid
            ...
            $sql = "SELECT count(observe.property_uuid) AS prop_count,
		properties.property_uuid,
		properties.val_num, properties.val_date
		FROM properties
		JOIN observe ON (observe.property_uuid = properties.property_uuid
		AND observe.subject_type LIKE '$querySubjectType')
		WHERE properties.variable_uuid = '".$this->varUUID."'
		GROUP BY properties.property_uuid
		ORDER BY prop_count DESC;
		";
	...
	if($this->dbPenelope){
            ...
            $sql = "SELECT count(observe.property_uuid) AS prop_count,
		properties.property_uuid,
		properties.val_num, properties.val_date, val_tab.val_text, val_tab.value_uuid
		FROM properties
		JOIN observe ON (observe.property_uuid = properties.property_uuid
		AND observe.subject_type LIKE '$querySubjectType')
		JOIN val_tab ON  val_tab.value_uuid = propertiesvalue_uuid
		WHERE properties.variable_uuid = '".$this->varUUID."'
		GROUP BY val_tab.val_text
		ORDER BY prop_count DESC;
		";
