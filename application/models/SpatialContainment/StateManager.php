    ...
    */
    ...
    public function transformCurrentNode()
    {
        ...
        */
        ...
        foreach($aliasRows as $actAlias){
            ...
            if($fieldName == $parentFieldName){
                $parentAlias = $actAlias["field_lab_com"];
            }
            
            if($fieldName == $childFieldName){
                $childAlias = $actAlias["field_lab_com"];
            }
       ...
       // echo "<br/><br/> $sql ($childFieldNum)";
        ...
        //echo "<br/><br/> FOund child? $foundChNum";
        ...
        */
        ...
        else
        {
            ...
            for($k=0; $k < $stopIdx; ++$k)
            {
                if($dataRow[$fieldList[$k]] != NULL){
                    $select->where($fieldList[$k] . ' = ?', $dataRow[$fieldList[$k]]);
                }
                else{
                    //avoid sql syntax error for blank values
                    $select->where($fieldList[$k] . ' IS NULL');
                    //$select->where($fieldList[$k] . ' = ?', $dataRow[$fieldList[$k]]);
                }
                ...
                //echo $fullContext;
                /*
                if($aliasList[$k] != null && strlen($aliasList[$k] > 0))
                    $fullContext .= $aliasList[$k] . ' ';
                */
                ...
                $fullContext .= trim($dataRow[$fieldList[$k]]);
            }
            $fullContext = $this->doubleDelimiterFix($fullContextDelimiter, $fullContext);
        ...
        catch(Exception $e)
        {
            ...
            echo chr(13).chr(13)."Eric added.".chr(13).chr(13);
            Zend_Debug::dump($so);
            return;
        }
        ...
        if($processDataRow){
            ...
            $parentDataItem = trim($dataRow[$parentFieldName]);
            if(strlen($fullContextCurrent) == 0)
            {   if($parentAlias != null && strlen($parentAlias > 0))
                    $fullContextCurrent .= $parentAlias . ' ';
                $fullContextCurrent .= $parentDataItem;
            ...
            $fullContextCurrent = $this->doubleDelimiterFix($fullContextDelimiter, $fullContextCurrent);
            ...
            if($spaceRow == null)
            {
                //insert parent into space
                $parentUUID             = GenericFunctions::generateUUID();
                $spaceLabel = trim($parentDataItem);
                if($parentAlias != null && !strpos($spaceLabel, $fullContextDelimiter)){
                    $spaceLabel = $parentAlias . ' ' . $spaceLabel;
                }
                
                $spaceLabel = str_replace($parentAlias . ' '.$parentAlias . ' ', $parentAlias . ' ', $spaceLabel);
                ...
                if($level == 0)// $level==0 means it's the top-level node
                {
                    ...
                    $fc = $proj_root_name . $fullContextDelimiter . $spaceLabel;
                }
                else
                {
                ...
                $fc = $this->doubleDelimiterFix($fullContextDelimiter, $fc);
            ...
            $childDataItem      = trim($dataRow[$childFieldName]);
            
            if($childAlias != null){
                $labelChildItem = $childAlias . ' ' . $childDataItem;
                $labelChildItem = str_replace($childAlias . ' '.$childAlias . ' ', $childAlias . ' ', $labelChildItem);
            }
            else{
                $labelChildItem = $childDataItem;
            }
            
            $fullContextChild   = $fullContextCurrent . $fullContextDelimiter . $labelChildItem;
            $fullContextChild = $this->doubleDelimiterFix($fullContextDelimiter, $fullContextChild);
            $hashTxt            = md5($projectUUID . "_" . $fullContextChild);
            //check to see if the child record exists in the space table:
            $spaceRow           = $space->fetchRow("hash_fcntxt = '" . $hashTxt . "'");
            
            if($spaceRow == null){
                //echo $fullContextChild;
                $fullContextChild = $proj_root_name.$fullContextChild;
            ...
            if($spaceRow == null)
            {
                ...
                if($childAlias != null){
                    $spaceLabel = $childAlias . ' ' . $spaceLabel;
                    $spaceLabel = str_replace($childAlias . ' '.$childAlias . ' ', $childAlias . ' ', $spaceLabel);
                }
            ...
            for($k=0; $k < $stopIdx; ++$k){
                if($dataRow[$fieldList[$k]] != NULL){
                    $selectLU->where($fieldList[$k] . ' = ?', $dataRow[$fieldList[$k]]);
                     ...
                     $selectLU->where($fieldList[$k] . ' IS NULL ');
                }
            }
        }
   ...
   */
    ...
    private function getFieldRows($dataTableName, $fieldName)
    {
        ...
        ->joinLeft(
               ...
               array('parent_alias' => 'field_label', 'parent_class' => 'fk_class_uuid',
        ...
        ->joinLeft(
               ...
               array('child_alias' => 'field_label', 'child_class' => 'fk_class_uuid',
