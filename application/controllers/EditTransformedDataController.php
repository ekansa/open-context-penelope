...
class EditTransformedDataController extends Zend_Controller_Action {
    ...
    */
    ...
    //this function changes parent child relations
    function changeParentsAction(){
        $this->_helper->viewRenderer->setNoRender();
        $childID = $_REQUEST['childID'];
        $newParentID = $_REQUEST['newParentID'];
        $projectUUID = $_REQUEST['projectUUID'];
        $returnVal = $this->execute_change_parents($childID, $newParentID, $projectUUID);
        echo $returnVal;
    }
    
    private function execute_change_parents($childID, $newParentID, $projectUUID){
        ...
        $where[] = "parent_uuid  = '".$newParentID."' ";
        $where[] = "child_uuid  = '".$childID."' ";
        ...
        $select = $db->select()
            ...
            ->from(
                    array('c' => 'space_contain'),
                    array('c.project_id', 'c.source_id', 'c.hash_all', 'c.parent_uuid', 'c.child_uuid')
            )
            ->where('c.child_uuid = ?', $childID)
            ->where ('c.project_id = ?', $projectUUID);
        $dataRows = $db->query($select)->fetchAll();
        ...
        foreach ($dataRows as $dataRow) 
        {
            ...
            $parent_UUID = $dataRow['parent_uuid'];
        ...
        $new_hash_all = md5($newParentID . '_' .$childID);
    ...
    function multipersonsplitAction(){
        ...
        if(count($_REQUEST['goodID'])==1){
            if(substr_count($good_pers_id_array,",")>0){
                ...
                $good_pers_id_array = array('0' => $good_pers_id_array);
            }
        }
