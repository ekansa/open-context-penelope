...
<xsl:variable name="num_editors">
	<xsl:value-of select="count(arch:person/oc:metadata/dc:creator)"/>
</xsl:variable>
				...
				<div id="item_top_tab">
						<div id="item_top_row">
								<div id="item_top_icon_cell">
										<img width='40' height='40'><xsl:attribute name="src">http://opencontext.org/images/item_view/project_icon.jpg</xsl:attribute><xsl:attribute name="alt">Project or Organization</xsl:attribute></img>
								</div>
								<div id="item_top_pers_name_cell">
										<h1><xsl:value-of select="arch:person/arch:name/arch:string"/></h1>
										<h2>Project: <a><xsl:attribute name="href">project?UUID=<xsl:if test="//arch:person/@ownedBy !=0"><xsl:value-of select="//arch:person/@ownedBy"/></xsl:if></xsl:attribute><xsl:value-of select="//arch:person/oc:metadata/oc:project_name"/></a>
										</h2>
								</div>       
								
								<div id="item_top_view_cell">Number of Views: <strong><xsl:value-of select="arch:person/oc:social_usage/oc:item_views[@type!='spatialCount']/oc:count"/></strong>
								</div>
						</div>
				</div>
		</div>
						...
						<div id="main_description_row">
								<div id="left_des">
										...
										<div id="proj-all-des">
											<h5>Description</h5>
												...
												<div id="abstract">
														<xsl:if test="count(descendant::arch:person/arch:notes/arch:note) = 0 and count(descendant::arch:person/oc:metadata/oc:links/oc:link) = 0" >
														...
														<xsl:if test="count(descendant::arch:person/oc:metadata/oc:links/oc:link) != 0" >
																<div id="person-links">
										...
										<xsl:if test="count(descendant::arch:person/arch:observations/arch:observation/arch:links/oc:space_links/oc:link) != 0" >
											<div id="all_links">
												<p class="subHeader">Linked Items (<xsl:value-of select="count(descendant::arch:person/arch:observations/arch:observation/arch:links/oc:space_links/oc:link)"/> items)</p>
													<xsl:for-each select="arch:person/arch:observations/arch:observation/arch:links/oc:space_links/oc:link">
															<xsl:choose>
										...
										<xsl:if test="count(descendant::arch:person/arch:properties/arch:property[oc:show_val/text()]) !=0 ">
												<div class="properties">
														<h5>Description (<xsl:value-of select="count(descendant::arch:person/arch:properties/arch:property[oc:show_val/text()])"/> properties)</h5>
														<table class="table table-striped table-condensed table-hover table-bordered prop-tab">
																...
																<tbody> 
																		<xsl:for-each select="arch:person/arch:properties/arch:property[oc:show_val/text()]">
																				<tr>
																						<td>
										...
										<xsl:if test="count(descendant::arch:person/arch:links/oc:person_links/oc:link) != 0">
												<div id="all_people" class="bodyText">
													<h5>Associated People (<xsl:value-of select="count(descendant::arch:person/arch:links/oc:person_links/oc:link)"/> people)</h5>
													<xsl:if test="count(descendant::arch:person/arch:links/oc:person_links/oc:link[oc:name/text() !='']) != 0" >	
														<p class="bodyText">
															<xsl:for-each select="arch:person/arch:links/oc:person_links/oc:link[oc:name/text() !='']">
																<a><xsl:attribute name="href">person?UUID=<xsl:value-of select="oc:id"/></xsl:attribute><xsl:value-of select="oc:name"/></a><xsl:if test="position() != last()">, </xsl:if>
															</xsl:for-each>
														</p>
													</xsl:if>
													<br/>
								...
								<div id="right_des">
										...
										<div id="all_media" class="bodyText" >
												<h5>Linked Media  (<xsl:value-of select="count(descendant::arch:person/arch:links/oc:media_links/oc:link)"/> files)</h5>
