...
<xsl:variable name="num_sp_obs">
	<xsl:value-of select="arch:property/oc:manage_info/oc:propStats[@observeType='Spatial' or @observeType='spatialUnit']/oc:varTotalObs"/>
</xsl:variable>
...
<link href="http://opencontext.org/css/default_spatial.css" rel="stylesheet" type="text/css" />
<link href="http://opencontext.org/css/default_property.css" rel="stylesheet" type="text/css" />
...
<div id="oc_logo">
	<a href="../" title="Open Context (Home)"><img alt="Open Context Logo" src="http://opencontext.org/images/general/oc_logo.jpg" border="0" ></img></a>
    </div>
    <div id="oc_tagline">
...
<div id="item_general"> 
    ...
    <div id="item_name_class"> 
      <div id="item_class_icon"> 
          <img width='40' height='40'> 
            <xsl:attribute name="src">http://opencontext.org/images/item_view/property_icon.jpg</xsl:attribute>
            <xsl:attribute name="alt">Descriptive Property</xsl:attribute>
          </img></div>
		<div id="item_name" class="subHeader">Descriptive Property</div>       
    ...
    <div id="viewtrack">
            ...
            <div id="item_last_view" class="tinyText">Project: <a><xsl:attribute name="href">../projects/<xsl:value-of select="arch:property/@ownedBy"/></xsl:attribute><xsl:value-of select="arch:property/oc:metadata/oc:project_name"/></a></div>
    </div>
</div>
<xsl:comment>
...
<div id="left_des">
	...
	<div id="allnotes" class="bodyText">
		...
		<xsl:choose>
		<xsl:when test="//oc:manage_info/oc:propVariable/@href | //arch:notes/arch:note[@type='var_des']" >
				
				<xsl:if test="//oc:manage_info/oc:propVariable/@href">
				<p class="bodyText">Unit of Measurement: <a>
																<xsl:attribute name="href">
																		<xsl:value-of select="//oc:manage_info/oc:propVariable/@href" />
																</xsl:attribute>
																<xsl:value-of select="//oc:manage_info/oc:propVariable/@name" />
														</a> (<xsl:value-of select="//oc:manage_info/oc:propVariable/@abrv" />)
																
				</p>
				</xsl:if>
				
				<xsl:for-each select="arch:property/arch:notes/arch:note[@type='var_des']">
						<p class="bodyText"><xsl:value-of select="arch:string" disable-output-escaping="yes" /></p><br/>
				</xsl:for-each>
				
		</xsl:when>
		<xsl:otherwise>
				<p class="bodyText">No exmplanatory information currently available</p>
		</xsl:otherwise>
		</xsl:choose>
		</div>
		
		<br/><br/>
		<p class="bodyText">Within the context of the "<xsl:value-of select="arch:property/oc:metadata/oc:project_name"/>" project, the variable <em><xsl:value-of select="//oc:manage_info/oc:propVariable"/></em> has <xsl:value-of select="//oc:numUniqueVals"/> unique values.
		<xsl:if test="//oc:valueRank" >The value "<xsl:value-of select="//oc:propValue"/>" has a frequency rank of <xsl:value-of select="//oc:valueRank"/> for this variable. </xsl:if>
		<xsl:if test="//oc:propMean" >The average value is: <xsl:value-of select="//oc:propMean"/></xsl:if>
		<xsl:if test="//oc:freqRank" >The value <em><xsl:value-of select="arch:property/arch:name/arch:string"/></em> is ranked number <strong><xsl:value-of select="//oc:freqRank"/></strong> in frequency.</xsl:if>
		</p>
		<br/>
		
		<xsl:if test="arch:property/oc:manage_info/oc:propStats[@observeType='Spatial' or @observeType='spatialUnit']" >	
			<p class="bodyText">
			<table>
				<tbody>
					<tr>
					...
					<xsl:for-each select="arch:property/oc:manage_info/oc:propStats[@observeType='Spatial' or @observeType='spatialUnit']/oc:graphData/oc:bar">
						<tr>
							<td><a><xsl:attribute name="href">../preview/prop-check?<xsl:value-of select="./@setURI"/></xsl:attribute><xsl:value-of select="."/></a></td><td><div><xsl:attribute name="style">margin-left:5px; height: 26px; text-align:center; padding-top:12px; background-color: #DCDCDC; width:<xsl:value-of select="round(((./@count) div $max_sp_obs)*400)"/>px;</xsl:attribute>(<xsl:value-of select="./@count"/>)</div></td>
						</tr>
					</xsl:for-each>
				</tbody>
			</table>
		...
		<xsl:if test="arch:property/arch:notes/arch:note[@type='prop_des']">
				<p class="bodyText"><strong>Notes for this Value: <em><xsl:value-of select="arch:property/arch:name/arch:string"/></em></strong></p>
				<xsl:for-each select="arch:property/arch:notes/arch:note[@type='prop_des']">
					<p class="bodyText"><xsl:value-of select="arch:string" disable-output-escaping="yes" /></p><br/>
				</xsl:for-each>
		</xsl:if>
	...
	<div id="properties">
		<p class="subHeader">Description (<xsl:value-of select="count(descendant::arch:property/arch:properties/arch:property)"/> properties)</p>
		<table border="0" cellpadding="1">
			 <xsl:for-each select="arch:property/arch:properties/arch:property">
				  <tr>
					<td width='95'>
			  ...
			  <xsl:if test="count(descendant::arch:property/arch:properties/arch:property) = 0">
				<tr><td><xsl:value-of select="arch:property/oc:metadata/oc:no_props" disable-output-escaping="yes"/></td></tr>
			  </xsl:if>
		</table>
	</div>
	...
	<div id="all_people" class="bodyText">
		<p class="subHeader">Associated People (<xsl:value-of select="count(descendant::arch:property/arch:links/oc:person_links/oc:link)"/> Persons)</p>
		<xsl:if test="count(descendant::arch:property/arch:links/oc:person_links/oc:link) != 0" >	
			<p class="bodyText">
				<xsl:for-each select="arch:property/arch:links/oc:person_links/oc:link">
...
<div id="right_des">
	...
	<div id="all_root" class="bodyText">
		<p class="subHeader">View Items with this Property</p>
			<a><xsl:attribute name="href">../sets/?<xsl:value-of select="//oc:manage_info/oc:queryVal"/></xsl:attribute><xsl:attribute name="title">Query for this property</xsl:attribute><!--<xsl:value-of select="//oc:manage_info/oc:propVariable"/>: --><xsl:value-of select="arch:property/arch:name/arch:string"/></a>
		<br/>	
	</div>
	...
	<div id="all_media" class="bodyText" align="left">
		<p class="subHeader">Linked Data</p>
		
		<xsl:choose>
		<xsl:when test="//oc:linkedData/oc:relationLink" >
		...
		<xsl:otherwise>
				<p class="bodyText">No linking relationships yet established</p>
		</xsl:otherwise>
		</xsl:choose>
	</div>
...
<div id="lic_pict">
<a>
	<xsl:attribute name="href"><xsl:value-of select="arch:property/oc:metadata/oc:copyright_lic/oc:lic_URI"/></xsl:attribute>
	<img width='88' height='31' border='0'> 
	  <xsl:attribute name="src"><xsl:value-of select="arch:property/oc:metadata/oc:copyright_lic/oc:lic_icon_URI"/></xsl:attribute>
	  <xsl:attribute name="alt"><xsl:value-of select="arch:property/oc:metadata/oc:copyright_lic/oc:lic_name"/></xsl:attribute>
	</img>
</a>
</div>
...
To the extent to which copyright applies, this content is licensed with:<a>
		...
		<xsl:attribute name="href"><xsl:value-of select="arch:property/oc:metadata/oc:copyright_lic/oc:lic_URI"/></xsl:attribute><xsl:value-of select="arch:property/oc:metadata/oc:copyright_lic/oc:lic_name"/>
            <xsl:value-of select="arch:property/oc:metadata/oc:copyright_lic/oc:lic_vers"/>&#32;License
