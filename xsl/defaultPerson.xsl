...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gml="http://www.opengis.net/gml" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:georss="http://www.georss.org/georss" xmlns:oc="http://about.opencontext.org/schema/person_schema_v1.xsd" xmlns:arch="http://ochre.lib.uchicago.edu/schema/Person/Person.xsd" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:dc="http://purl.org/dc/elements/1.1/">
...
<link href="/css/default_person.css" rel="stylesheet" type="text/css" />
...
<div id="oc_main_nav">
        ...
        <div class="sides"></div>
        <div class="act_nav"><a href="" title="Viewing details of a person">Details</a></div>
        <div class="sides"></div>
...
<div id="item_general"> 
    ...
    <div id="item_name_class"> 
      <div id="item_class_icon"> 
          <img width='40' height='40'> 
            <xsl:attribute name="src">/images/item_view/person_icon.jpg</xsl:attribute>
            <xsl:attribute name="alt">Person or Organization</xsl:attribute>
          </img></div>
       <div id="item_name" class="subHeader">Person: <span class="bodyText"><xsl:value-of select="atom:feed/atom:entry/arch:person/arch:name/arch:string"/></span></div>
       <div id="item_class" class="subHeader">Project: <span class="bodyText"><xsl:value-of select="atom:feed/atom:entry/arch:person/oc:metadata/oc:project_name"/></span></div>
    </div>
    ...
    <div id="viewtrack">
            ...
            <div id="item_last_view" class="tinyText">Project: <a><xsl:attribute name="href">../projects/<xsl:value-of select="atom:feed/atom:entry/arch:person/@ownedBy"/></xsl:attribute><xsl:value-of select="atom:feed/atom:entry/arch:person/oc:metadata/oc:project_name"/></a></div>
    </div>
</div>
<xsl:comment>
...
<div id="left_des">
	...
	<div id="properties">
		<p class="subHeader">Description (<xsl:value-of select="count(descendant::atom:feed/atom:entry/arch:person/arch:properties/arch:property)"/> properties)</p>
		<table border="0" cellpadding="1">
			 <xsl:for-each select="atom:feed/atom:entry/arch:person/arch:properties/arch:property">
				  <tr>
					<td width='95'>
			  ...
			  <xsl:if test="count(descendant::atom:feed/atom:entry/arch:person/arch:properties/arch:property) = 0">
				<tr><td><xsl:value-of select="atom:feed/atom:entry/arch:person/oc:metadata/oc:no_props" disable-output-escaping="yes"/></td></tr>
			  </xsl:if>
		</table>
	</div>
	...
	<div id="allnotes" class="bodyText">
		...
		<xsl:if test="count(descendant::atom:feed/atom:entry/arch:person/arch:observations/arch:observation/arch:links/oc:diary_links/oc:link) != 0" >	
			<p class="bodyText">
				<xsl:for-each select="atom:feed/atom:entry/arch:person/arch:observations/arch:observation/arch:links/oc:diary_links/oc:link">
		...
		<xsl:if test="count(descendant::atom:feed/atom:entry/arch:person/oc:social_usage/oc:external_references/oc:reference) != 0" >	
			<p class="bodyText"> 
				<xsl:for-each select="atom:feed/atom:entry/arch:person/oc:social_usage/oc:external_references/oc:reference">
	...
	<xsl:if test="count(descendant::atom:feed/atom:entry/arch:person/arch:links/oc:space_links/oc:link) != 0" >
		<div id="all_links">
			<p class="subHeader">Linked Items (<xsl:value-of select="count(descendant::atom:feed/atom:entry/arch:person/arch:links/oc:space_links/oc:link)"/> items)</p>
				<xsl:for-each select="atom:feed/atom:entry/arch:person/arch:links/oc:space_links/oc:link">
						<xsl:choose>
	...
	<div id="all_people" class="bodyText">
		<p class="subHeader">Associated People (<xsl:value-of select="count(descendant::atom:feed/atom:entry/arch:person/arch:links/oc:person_links/oc:link)"/> items)</p>
		<xsl:if test="count(descendant::atom:feed/atom:entry/arch:person/arch:links/oc:person_links/oc:link) != 0" >	
			<p class="bodyText">
				<xsl:for-each select="atom:feed/atom:entry/arch:person/arch:links/oc:person_links/oc:link">
...
<div id="right_des">
	...
	<div id="all_tags" class="bodyText">
		<p class="subHeader">Tags Made by Other Users  (<xsl:value-of select="count(descendant::atom:feed/atom:entry/atom:category[@term='user tag'])"/>)</p>
		<p class="bodyText">Items associated with this person have been tagged by: <strong><xsl:value-of select="	count(descendant::atom:feed/atom:entry/atom:category[@term='tag creator'])"/></strong> people.</p>
			<xsl:for-each select="atom:feed/atom:entry">
	...
	<div id="all_media" class="bodyText" align="left">
		<p class="subHeader">Linked Media  (<xsl:value-of select="count(descendant::atom:feed/atom:entry/arch:person/arch:links/oc:media_links/oc:link)"/> files)</p>
