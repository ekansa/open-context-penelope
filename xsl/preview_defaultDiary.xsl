...
-
    ...
    <div id="parent_contexts">
        ...
        <div id="pcontext" class="bodyText" align="left">
                    ...
                    <a><xsl:attribute name="href">space?UUID=<xsl:value-of select="arch:resource/arch:links/oc:space_links/oc:link/oc:id"/></xsl:attribute><xsl:value-of select="arch:resource/arch:links/oc:space_links/oc:link/oc:name"/></a> (Linked item)
		...
		<xsl:if test="count(descendant::arch:resource/arch:links/oc:person_links/oc:link) != 0" >	
			<p class="bodyText">
				<xsl:for-each select="arch:resource/arch:links/oc:person_links/oc:link">
				   <a><xsl:attribute name="href">../preview/person?UUID=<xsl:value-of select="oc:id"/></xsl:attribute><xsl:value-of select="oc:name"/></a> ( <xsl:value-of select="oc:relation"/> )<xsl:if test="position() != last()"> , </xsl:if>
				</xsl:for-each>
			</p>
		</xsl:if>
		<br/>
...
<div id="right_des">
	...
	<div id="all_tags" class="bodyText">
		<p class="subHeader">User Generated Tags  (<xsl:value-of select="count(descendant::arch:resource/oc:social_usage/oc:user_tags/oc:tag[@status='public'])"/>)</p>
			<h3>[DRAFT / PREVIEW CONTENT - NOT PUBLISHED]</h3>
			<p class="bodyText">
				<xsl:for-each select="arch:resource/oc:social_usage/oc:user_tags/oc:tag[@origin_id=$firstOrigin]">
				   <a>
	...
	<div id="all_media" class="bodyText" align="left">
		...
		<xsl:if test="count(descendant::arch:resource/arch:links/oc:diary_links/oc:link) != 0" >
				<p class="subHeader">Linked Narratives  (<xsl:value-of select="count(descendant::arch:resource/arch:links/oc:diary_links/oc:link)"/> Docs)</p>
				<table>
