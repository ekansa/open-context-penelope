...
<title>Open Context view of <xsl:value-of select="atom:title"/></title>

<link rel="shortcut icon" href="/images/general/oc_favicon.ico" type="image/x-icon" />
<link rel="alternate" type="application/atom+xml">
<xsl:attribute name="title">Atom feed: <xsl:value-of select="atom:title"/></xsl:attribute>
<xsl:attribute name="href"><xsl:value-of select="atom:id"/>.atom</xsl:attribute>
</link>
...
<link href="http://opencontext.org/css/default_project.css" rel="stylesheet" type="text/css" />
    ...
    <div id="item_name_class"> 
      <div id="item_class_icon"> 
          <img width='40' height='40'> 
            <xsl:attribute name="src">http://opencontext.org/images/item_view/project_icon.jpg</xsl:attribute>
            <xsl:attribute name="alt">Project or Organization</xsl:attribute>
          </img></div>
<div id="item_name" class="subHeader">Project / Collection: <span class="bodyText"><xsl:value-of select="arch:project/arch:name/arch:string"/></span></div>       
       <div id="item_class" class="subHeader">Description: <span class="bodyText"><xsl:value-of select="arch:project/arch:notes/arch:note[@type='short_des']" disable-output-escaping="yes"/></span></div>
    </div>
    ...
    <div id="viewtrack">
            <div id="item_views" class="bodyText">Number of Views: <strong><xsl:value-of select="arch:project/oc:social_usage/oc:item_views[@type!='spatialCount']/oc:count"/></strong></div>
...
<div id="left_des">
	...
	<div id="allnotes" class="bodyText">
		...
		<xsl:if test="count(descendant::arch:project/arch:observations/arch:observation/arch:links/oc:diary_links/oc:link) != 0" >	
			<p class="bodyText">
				<xsl:for-each select="arch:project/arch:observations/arch:observation/arch:links/oc:diary_links/oc:link">
		...
		<xsl:if test="count(descendant::arch:project/oc:social_usage/oc:external_references/oc:reference) != 0" >	
			<p class="bodyText"> 
				<xsl:for-each select="arch:project/oc:social_usage/oc:external_references/oc:reference">
	...
	<div id="properties">
		<p class="subHeader">Description (<xsl:value-of select="count(descendant::arch:project/arch:properties/arch:property)"/> properties)</p>
		<table border="0" cellpadding="1">
			 <xsl:for-each select="arch:project/arch:properties/arch:property">
				  <tr>
					<td width='95'>
			  ...
			  <xsl:if test="count(descendant::arch:project/arch:properties/arch:property) = 0">
				<tr><td><xsl:value-of select="arch:project/oc:metadata/oc:no_props" disable-output-escaping="yes"/></td></tr>
			  </xsl:if>
		</table>
	</div>
	...
	<div id="all_people" class="bodyText">
		<p class="subHeader">Associated People (<xsl:value-of select="count(descendant::arch:project/arch:links/oc:person_links/oc:link)"/> items)</p>
		<xsl:if test="count(descendant::arch:project/arch:links/oc:person_links/oc:link) != 0" >	
			<p class="bodyText">
				<xsl:for-each select="arch:project/arch:links/oc:person_links/oc:link">
				   <a><xsl:attribute name="href">../preview/person?UUID=<xsl:value-of select="oc:id"/></xsl:attribute><xsl:value-of select="oc:name"/></a><xsl:if test="position() != last()"> , </xsl:if>
				</xsl:for-each>
			</p>
		</xsl:if>
		<br/>
...
<div id="right_des">
						...
						<tbody>
								<xsl:for-each select="//oc:space_links/oc:link">
										<xsl:if test="position() = 1">
												<tr>
														<td>
																<a><xsl:attribute name="href">../preview/space?UUID=<xsl:value-of select="oc:id"/></xsl:attribute>
																		<img>
																				<xsl:attribute name="src">http://www.opencontext.org/database/ui_images/oc_icons/<xsl:value-of select="oc:item_class/oc:iconURI"/></xsl:attribute>
																				<xsl:attribute name="alt"><xsl:value-of select="oc:item_class/oc:name"/></xsl:attribute>
																		</img>
	...
	<div id="all_tags" class="bodyText">
		<p class="subHeader">Tags Used in this Project  (<xsl:value-of select="count(descendant::atom:category[@term='user tag'])"/>)</p>
		<p class="bodyText">Items from this project/collection have been tagged by: <strong><xsl:value-of select="	count(descendant::atom:category[@term='tag creator'])"/></strong> people.</p>
			<xsl:for-each select="atom:feed/atom:entry">
	...
	<div id="all_media" class="bodyText" >
		<p class="subHeader">Linked Media  (<xsl:value-of select="count(descendant::arch:project/arch:links/oc:media_links/oc:link)"/> files)</p>
...
<div id="lic_pict">
<a>
	<xsl:attribute name="href"><xsl:value-of select="arch:project/oc:metadata/oc:copyright_lic/oc:lic_URI"/></xsl:attribute>
	<img width='88' height='31' style='border:none;'> 
	  <xsl:attribute name="src"><xsl:value-of select="arch:project/oc:metadata/oc:copyright_lic/oc:lic_icon_URI"/></xsl:attribute>
	  <xsl:attribute name="alt"><xsl:value-of select="arch:project/oc:metadata/oc:copyright_lic/oc:lic_name"/></xsl:attribute>
	</img>
</a>
</div>
...
To the extent to which copyright applies, this content is licensed with:<a>
		...
		<xsl:attribute name="href"><xsl:value-of select="arch:project/oc:metadata/oc:copyright_lic/oc:lic_URI"/></xsl:attribute><xsl:value-of select="arch:project/oc:metadata/oc:copyright_lic/oc:lic_name"/>
            <xsl:value-of select="arch:project/oc:metadata/oc:copyright_lic/oc:lic_vers"/>&#32;License
	...
	<div style="width:0px; overflow:hidden;">
		<a xmlns:cc="http://creativecommons.org/ns#">
			<xsl:attribute name="href"><xsl:value-of select="arch:projectt/oc:metadata/dc:identifier"/></xsl:attribute>
