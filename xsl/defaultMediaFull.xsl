...
<xsl:attribute name="href"><xsl:value-of select="arch:resource/@UUID"/>.atom</xsl:attribute>
</link>
		...
		<xsl:for-each select="arch:resource/arch:links/oc:space_links/oc:link/oc:context/oc:tree[@id='default']">
				...
				Context: 
						   <xsl:for-each select="oc:parent">
							<a><xsl:attribute name="href">http://opencontext.org/subjects/<xsl:value-of select="oc:id"/></xsl:attribute><xsl:value-of select="oc:name"/></a> /
							</xsl:for-each> 
						</xsl:if>
                    </xsl:for-each>
                    
                    <a><xsl:attribute name="href">http://opencontext.org/subjects/<xsl:value-of select="arch:resource/arch:links/oc:space_links/oc:link/oc:id"/></xsl:attribute><xsl:value-of select="arch:resource/arch:links/oc:space_links/oc:link/oc:name"/></a> (Linked item)
		...
		<table style="border:none; padding:1px;">
			 <xsl:for-each select="arch:resource/arch:properties/arch:property">
				  <tr>
					...
					<td>
						<a>
							<xsl:attribute name="href">http://opencontext.org/properties/<xsl:value-of select="oc:propid"/></xsl:attribute>
							<xsl:choose>
