...
<script type="text/javascript" src="../public/scripts/tabber.js"></script>
...
<div id="left_des">
	...
	<xsl:choose>
		...
		<xsl:for-each select="atom:feed/atom:entry/arch:spatialUnit/arch:observations/arch:observation">
				<div>
				<xsl:choose>
				<xsl:when test="$num_Obs = 1">
						<xsl:attribute name="style">margin-top:10px;</xsl:attribute>
				</xsl:when>
				<xsl:when test="(@obsNumber = '100') or (oc:obs_metadata/oc:type = 'Preliminary')">
						...
						<xsl:param name="prelimData">1</xsl:param>
				</xsl:when>
				...
				<xsl:otherwise>
						<!-- <xsl:attribute name="style">background-color: #F5F5DC;</xsl:attribute> -->
				</xsl:otherwise>
				</xsl:choose>		
				<xsl:choose>
				<xsl:when test="$num_Obs = 1">
				...
				<div class="obsProps rounded-corners">
					<p class="subHeader">Description (<xsl:value-of select="count(descendant::arch:properties/arch:property)"/> properties)</p>
		...
		<xsl:if test="count(descendant::arch:links/oc:person_links/oc:link) != 0" >	
			<p class="bodyText">
				<xsl:for-each select="arch:links/oc:person_links/oc:link">
				   <a><xsl:attribute name="href">http://about.oc/oc_xmlgen/person.php?item=<xsl:value-of select="oc:id"/>&amp;imp=true</xsl:attribute><xsl:value-of select="oc:name"/></a> ( <xsl:value-of select="oc:relation"/> )<xsl:if test="position() != last()"> , </xsl:if>
				</xsl:for-each>
			</p>
		</xsl:if>
		<br/>
...
<div id="right_des">
	...
	<div id="all_tags" class="bodyText rounded-corners">
		<p class="subHeader">Descriptive Tags  (<xsl:value-of select="count(descendant::atom:feed/atom:entry/arch:spatialUnit/oc:social_usage/oc:user_tags/oc:tag[@status='public'])"/>)</p>
			<p class="bodyText">
	...
	<div id="all_media" class="bodyText rounded-corners" align="left">
		...
		<br/>
		<a>
				<xsl:attribute name="href">space?UUID=<xsl:value-of select="//arch:spatialUnit/@UUID"/>&amp;format=edit</xsl:attribute>Edit this item
		</a>
		<br/>
		<br/>
		<br/>
		<a>
				<xsl:attribute name="href">http://penelope.oc/publish/publishdoc?projectUUID=<xsl:value-of select="//arch:spatialUnit/@ownedBy"/>&amp;itemUUID=<xsl:value-of select="//arch:spatialUnit/@UUID"/>&amp;itemType=space</xsl:attribute>New Publish on Local Server
		</a>
		<br/>
		<a>
				<xsl:attribute name="href">http://penelope.oc/publish/publishdoc?projectUUID=<xsl:value-of select="//arch:spatialUnit/@ownedBy"/>&amp;itemUUID=<xsl:value-of select="//arch:spatialUnit/@UUID"/>&amp;itemType=space&amp;doUpdate=true</xsl:attribute>Update Publish on Local Server
		</a>
		<br/>
		<a>
		...
		<br/>
		<a>
				<xsl:attribute name="href">http://penelope.oc/publish/publishdoc?projectUUID=<xsl:value-of select="//arch:spatialUnit/@ownedBy"/>&amp;itemUUID=<xsl:value-of select="//arch:spatialUnit/@UUID"/>&amp;itemType=space&amp;pubURI=http://opencontext.org/publish/itempublish</xsl:attribute>New Publish on Open Context
		</a>
		<br/>
		<a>
				<xsl:attribute name="href">http://penelope.oc/publish/publishdoc?projectUUID=<xsl:value-of select="//arch:spatialUnit/@ownedBy"/>&amp;itemUUID=<xsl:value-of select="//arch:spatialUnit/@UUID"/>&amp;itemType=space&amp;doUpdate=true&amp;pubURI=http://opencontext.org/publish/itempublish</xsl:attribute>Update Publish on Open Context
		</a>
		<br/>
		<a>
		...
		<br/>
		<a>
				<xsl:attribute name="href">http://penelope.oc/importer/edit-dataset/set-prop-change?projectUUID=<xsl:value-of select="//arch:spatialUnit/@ownedBy"/>&amp;itemUUID=<xsl:value-of select="//arch:spatialUnit/@UUID"/></xsl:attribute>Edit Properties, Notes, Etc.
		</a>
		<br/>
		<br/>
		<a>
				<xsl:attribute name="href">http://penelope.oc/publish/?projectUUID=<xsl:value-of select="//arch:spatialUnit/@ownedBy"/></xsl:attribute>Go to Project Publishing Page 
		</a>
	</div>
...
<xsl:template name="dots">
 ...
 <xsl:param name="notCurrent" select="1"/>
