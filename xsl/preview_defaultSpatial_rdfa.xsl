...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:owl="http://www.w3.org/2002/07/owl#" xmlns:ocsem="http://opencontext.org/about/concepts#"
 xmlns:conc="http://gawd.atlantides.org/terms/" xmlns:gml="http://www.opengis.net/gml" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:georss="http://www.georss.org/georss" xmlns:oc="http://opencontext.org/schema/space_schema_v1.xsd" xmlns:arch="http://ochre.lib.uchicago.edu/schema/SpatialUnit/SpatialUnit.xsd" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:dc="http://purl.org/dc/elements/1.1/">
...
<div id="left_des">
	...
	<xsl:choose>
		...
		<xsl:for-each select="atom:feed/atom:entry/arch:spatialUnit/arch:observations/arch:observation">
				...
				<xsl:choose>
						<xsl:when test="(@obsNumber = '100') or (@obsNumber &lt; '0') or (oc:obs_metadata/oc:type = 'Preliminary') ">
	...
	<div id="allchildren">
		...
		<br/>
		<br/>
		<p class="subHeader">Linked Documents  (<xsl:value-of select="count(descendant::atom:feed/atom:entry/arch:spatialUnit/arch:observations/arch:observation/arch:links/oc:diary_links/oc:link)"/> files)</p>
...
<div id="right_des">
	...
	<div id="all_media" class="bodyText rounded-corners" align="left">
		...
		<br/>
		<a>
				<xsl:attribute name="href">http://opencontext.org/publish/itempublish?index=true</xsl:attribute>Committ Changes on Local Server
		</a>
		...
		<br/>
		<a>
				<xsl:attribute name="href">http://penelope2.oc/importer/edit-dataset/set-prop-change?projectUUID=<xsl:value-of select="//arch:spatialUnit/@ownedBy"/>&amp;itemUUID=<xsl:value-of select="//arch:spatialUnit/@UUID"/></xsl:attribute>Edit Properties, Notes, Etc.
		</a>
		<br/>
		<br/>
		<a>
...
<xsl:template name="obsShow">
		...
		<xsl:param name="propBaseURI">property?UUID=</xsl:param>
		...
		<xsl:if test="count(descendant::arch:links/oc:person_links/oc:link) != 0" >	
			<p class="bodyText">
				<xsl:for-each select="arch:links/oc:person_links/oc:link">
				   <a><xsl:attribute name="href">../preview/person?UUID=<xsl:value-of select="oc:id"/>&amp;imp=true</xsl:attribute><xsl:value-of select="oc:name"/></a> ( <xsl:value-of select="oc:relation"/> )<xsl:if test="position() != last()"> , </xsl:if>
				</xsl:for-each>
			</p>
		</xsl:if>
		<br/>
