...
<div id="left_des">
		...
		<xsl:for-each select="atom:feed/atom:entry/arch:spatialUnit/arch:observations/arch:observation">
				...
				<xsl:when test="(@obsNumber = '100') or (oc:obs_metadata/oc:type = 'Preliminary')">
						<xsl:attribute name="class">hideObs</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
						<xsl:attribute name="class">obs</xsl:attribute>
				</xsl:otherwise>
				</xsl:choose>
						<p class="tinyText">Observation: <xsl:value-of select="@obsNumber"/></p>
				<div id="properties">
				...
				<div id="allnotes" class="bodyText">
					...
					<xsl:if test="count(descendant::../oc:external_references/oc:reference) != 0" >	
						<p class="bodyText"> 
